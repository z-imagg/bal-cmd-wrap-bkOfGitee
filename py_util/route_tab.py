...
#编译器 拦截： 入口者 --> 业务者
fake_clang=pathNorm(f"{fakeBinHm}/clang") #编译器 入口者
...
fake_clangxx=pathNorm(f"{fakeBinHm}/clang++") #编译器 入口者
...
fake_gcc=pathNorm(f"{fakeBinHm}/gcc") #编译器 入口者
busz_gcc=pathNorm(f"/usr/bin/x86_64-linux-gnu-gcc-11") #编译器 业务者
progMap[fake_gcc]     = Prog(fake_gcc,busz_gcc,Prog.ProgKind.Compiler) # 入口者 --> 业务者

# 这组未测试
# fake_gxx=pathNorm(f"{fakeBinHm}/g++") #编译器 入口者
